...
Error: ${error.message}`;
  ...
  async function sendMessage() {
    ...
    const isConcerning = checkForConcerningMessage(message);
    if (isConcerning) {
      alertAdminDashboard(message);

      // Show immediate response for concerning messages
      typingIndicator.style.display = "none";
      addMessage(
        "I'm very concerned about what you just shared. Your location has been sent to emergency services and help is on the way. Please stay safe - you are not alone in this.",
        false
      );
      ...
      setTimeout(() => {
        ...
        addMessage("National Suicide Prevention Lifeline: 1-800-273-TALK (8255)", false);
        addMessage("Crisis Text Line: Text HOME to 741741", false);
        addMessage("You matter and there are people who want to help you through this.", false);
      }, 1000);

      return; // Skip the normal therapist response
    }
...
function setupAlertsDisplay() {
  ...
  alertBell.addEventListener("click", function () {
    alertsContainer.style.display =
      alertsContainer.style.display === "none" ? "block" : "none";
...
function clearAllAlerts() {
  if (
    confirm(
      "Are you sure you want to clear all crisis alerts? This cannot be undone."
    )
  ) {
  ...
  if (alertsList) {
    ...
    if (alerts.length === 0) {
      ...
      alerts.reverse().forEach((alert) => {
        ...
        time.innerHTML = `<strong>Time:</strong> ${new Date(
          alert.timestamp
        ).toLocaleString()}`;
        ...
        if (alert.location) {
          ...
          location.innerHTML = `<strong>Approximate Location:</strong> 
            <a href="https://www.google.com/maps?q=${alert.location.latitude},${
...
function showUserChatHistory(username) {
  ...
  if (!modal) {
    ...
    document
      .getElementById("close-chat-history")
